...
// A correção começa aqui, com a adição de collectionGroup
import { getFirestore, doc, setDoc, onSnapshot, collection, query, getDocs, getDoc, deleteDoc, collectionGroup } from 'firebase/firestore';
...
const App = () => {
 ...
 Verifique a configuração do Firebase. Detalhes: ${error.message}`,
        type: 'info',
        onConfirm: () => {},
        onCancel: () => {},
      });
  ...
  const fetchCharactersList = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      // A correção principal está neste bloco if (isMaster)
      if (isMaster) {
        console.log("fetchCharactersList: Modo Mestre, buscando todos os personagens com collectionGroup.");
        
        // Cria uma consulta em todas as coleções com o ID 'characterSheets'
        const allCharactersQuery = query(collectionGroup(db, 'characterSheets'));
        const querySnapshot = await getDocs(allCharactersQuery);
        
        // Mapeia os resultados, extraindo o ownerUid do caminho do documento
        const allChars = querySnapshot.docs
          .map(doc => ({
            id: doc.id,
            // O caminho é .../users/{ownerUid}/characterSheets/{charId}
            // doc.ref.parent nos dá a coleção 'characterSheets'
            // doc.ref.parent.parent nos dá o documento do usuário {ownerUid}
            ownerUid: doc.ref.parent.parent.id, 
            ...doc.data()
          }))
          .filter(char => !char.deleted); // Filtra fichas marcadas como deletadas
        ...
        unsubscribeCharacter = onSnapshot(characterDocRef, (docSnap) => {
          if (docSnap.exists()) {
            ...
            try {
              ...
              deserializedData.basicAttributes = typeof deserializedData.basicAttributes === 'string' ?
              JSON.parse(deserializedData.basicAttributes) : deserializedData.basicAttributes;
              ...
              deserializedData.history = Array.isArray(historyData) ?
              historyData.map(block => ({ ...block, isCollapsed: block.isCollapsed !== undefined ? block.isCollapsed : false })) : [];
              ...
              if (typeof notesData === 'string') { // Handle old string notes
                try {
                  ...
                  // Try parsing if it was already stringified JSON
                } catch (parseError) {
                  notesData = [{ id: crypto.randomUUID(), type: 'text', value: notesData }];
                  // Convert old string to a text block
                }
              }
              deserializedData.notes = Array.isArray(notesData) ?
              notesData.map(block => ({ ...block, isCollapsed: block.isCollapsed !== undefined ? block.isCollapsed : false })) : [];
            } catch (e) {
            ...
            deserializedData.isUserStatusCollapsed = data.isUserStatusCollapsed !== undefined ?
            data.isUserStatusCollapsed : false;
            deserializedData.isCharacterInfoCollapsed = data.isCharacterInfoCollapsed !== undefined ? data.isCharacterInfoCollapsed : false;
            deserializedData.isMainAttributesCollapsed = data.isMainAttributesCollapsed !== undefined ?
            data.isMainAttributesCollapsed : false;
            ...
            deserializedData.isInventoryCollapsed = data.isInventoryCollapsed !== undefined ?
            data.isInventoryCollapsed : false;
            deserializedData.isWalletCollapsed = data.isWalletCollapsed !== undefined ? data.isWalletCollapsed : false;
            deserializedData.isPerksCollapsed = data.isPerksCollapsed !== undefined ?
            data.isPerksCollapsed : false;
            deserializedData.isAbilitiesCollapsed = data.isAbilitiesCollapsed !== undefined ? data.isAbilitiesCollapsed : false;
            deserializedData.isSpecializationsCollapsed = data.isSpecializationsCollapsed !== undefined ?
            data.isSpecializationsCollapsed : false;
            deserializedData.isEquippedItemsCollapsed = data.isEquippedItemsCollapsed !== undefined ? data.isEquippedItemsCollapsed : false;
            deserializedData.isHistoryCollapsed = data.isHistoryCollapsed !== undefined ?
            data.isHistoryCollapsed : false;
            deserializedData.isNotesCollapsed = data.isNotesCollapsed !== undefined ? data.isNotesCollapsed : false;


            deserializedData.mainAttributes = deserializedData.mainAttributes ||
            { hp: { current: 0, max: 0 }, mp: { current: 0, max: 0 }, initiative: 0, fa: 0, fm: 0, fd: 0 };
            ...
            // Agora é um array
            deserializedData.level = deserializedData.level !== undefined ?
            deserializedData.level : 0;
            deserializedData.xp = deserializedData.xp !== undefined ? deserializedData.xp : 100;
            deserializedData.photoUrl = deserializedData.photoUrl || '';
            // Garante que photoUrl seja string vazia se não presente
  ...
  }, [db, user, isAuthReady, selectedCharIdState, ownerUidState, appId, isMaster, fetchCharactersList, setModal]);
  ...
  useEffect(() => {
    if (db && user && isAuthReady && character && selectedCharIdState) { // Usando o estado
      ...
      const characterDocRef = doc(db, `artifacts/${appId}/users/${targetUidForSave}/characterSheets/${selectedCharIdState}`); // Usando 
      // o estado
    ...
    setModal({
      ...
      onConfirm: () => {
        setCharacter({
          ...
          basicAttributes: { forca: { base: 0, permBonus: 0, condBonus: 0, total: 0 }, destreza: { base: 0, permBonus: 0, condBonus: 0, total: 0 }, inteligencia: { base: 0, permBonus: 0, condBonus: 0, 
          total: 0 }, constituicao: { base: 0, permBonus: 0, condBonus: 0, total: 0 }, sabedoria: { base: 0, permBonus: 0, condBonus: 0, total: 0 }, carisma: { base: 0, permBonus: 0, condBonus: 0, total: 0 }, armadura: { base: 0, permBonus: 0, condBonus: 0, total: 0 }, poderDeFogo: { base: 0, permBonus: 0, condBonus: 0, total: 0 } },
          magicAttributes: { fogo: { base: 0, permBonus: 0, condBonus: 0, total: 0 }, agua: { base: 0, permBonus: 0, condBonus: 0, total: 0 }, ar: { base: 0, permBonus: 0, condBonus: 0, 
          total: 0 }, terra: { base: 0, permBonus: 0, condBonus: 0, total: 0 }, luz: { base: 0, permBonus: 0, condBonus: 0, total: 0 }, trevas: { base: 0, permBonus: 0, condBonus: 0, total: 0 }, espirito: { base: 0, permBonus: 0, condBonus: 0, total: 0 }, outro: { base: 0, permBonus: 0, condBonus: 0, total: 0 } },
              ...
              onConfirm: async () => {
                ...
                const importedCharacterData = {
                  ...
                  mainAttributes: {
                    ...
                    initiative: importedData.mainAttributes?.initiative ||
                    0,
                    fa: importedData.mainAttributes?.fa ||
                    0,
                    fm: importedData.mainAttributes?.fm ||
                    0,
                    fd: importedData.mainAttributes?.fd ||
                    0,
                  },
                  basicAttributes: {
                  ...
                  magicAttributes: {
                    ...
                    trevas: { base: 
                    0, permBonus: 0, condBonus: 0, total: 0, ...importedData.magicAttributes?.trevas },
               ...
               inventory: importedData.inventory || [],
                  ...
                  isUserStatusCollapsed: importedData.isUserStatusCollapsed !== undefined ?
                  importedData.isUserStatusCollapsed : false,
                  isCharacterInfoCollapsed: importedData.isCharacterInfoCollapsed !== undefined ?
                  importedData.isCharacterInfoCollapsed : false,
                  isMainAttributesCollapsed: importedData.isMainAttributesCollapsed !== undefined ?
                  importedData.isMainAttributesCollapsed : false,
                  isBasicAttributesCollapsed: importedData.isBasicAttributesCollapsed !== undefined ?
                  importedData.isBasicAttributesCollapsed : false,
                  isInventoryCollapsed: importedData.isInventoryCollapsed !== undefined ?
                  importedData.isInventoryCollapsed : false,
                  isWalletCollapsed: importedData.isWalletCollapsed !== undefined ?
                  importedData.isWalletCollapsed : false,
                  isPerksCollapsed: importedData.isPerksCollapsed !== undefined ?
                  importedData.isPerksCollapsed : false,
                  isAbilitiesCollapsed: importedData.isAbilitiesCollapsed !== undefined ?
                  importedData.isAbilitiesCollapsed : false,
                  isSpecializationsCollapsed: importedData.isSpecializationsCollapsed !== undefined ?
                  importedData.isSpecializationsCollapsed : false,
                  isEquippedItemsCollapsed: importedData.isEquippedItemsCollapsed !== undefined ?
                  importedData.isEquippedItemsCollapsed : false,
                  isHistoryCollapsed: importedData.isHistoryCollapsed !== undefined ?
                  importedData.isHistoryCollapsed : false,
                  isNotesCollapsed: importedData.isNotesCollapsed !== undefined ?
                  importedData.isNotesCollapsed : false,
                };
          ...
          try {
            const newCharId = crypto.randomUUID();
             ...
             const newCharacterData = {
              ...
              level: 0, xp: 100,
              mainAttributes: { hp: { current: 0, max: 0 }, mp: { current: 0, max: 0 }, initiative: 0, fa: 0, fm: 0, fd: 0 },
              basicAttributes: { forca: { base: 0, permBonus: 0, condBonus: 0, total: 0 }, destreza: { base: 0, permBonus: 0, condBonus: 0, total: 0 
              }, inteligencia: { base: 0, permBonus: 0, condBonus: 0, total: 0 }, constituicao: { base: 0, permBonus: 0, condBonus: 0, total: 0 }, sabedoria: { base: 0, permBonus: 0, condBonus: 0, total: 0 }, carisma: { base: 0, permBonus: 0, condBonus: 0, total: 0 }, armadura: { base: 0, permBonus: 0, condBonus: 0, total: 0 }, poderDeFogo: { base: 0, permBonus: 0, condBonus: 0, total: 0 } },
              magicAttributes: { fogo: { base: 0, permBonus: 0, condBonus: 0, total: 0 }, agua: { base: 0, permBonus: 0, 
              condBonus: 0, total: 0 }, ar: { base: 0, permBonus: 0, condBonus: 0, total: 0 }, terra: { base: 0, permBonus: 0, condBonus: 0, total: 0 }, luz: { base: 0, permBonus: 0, condBonus: 0, total: 0 }, trevas: { base: 0, permBonus: 0, condBonus: 0, total: 0 }, espirito: { base: 0, permBonus: 0, condBonus: 0, total: 0 }, outro: { base: 0, permBonus: 0, condBonus: 0, total: 0 } },
              inventory: [], wallet: { zeni: 0 }, advantages: [], disadvantages: [], abilities: [], specializations: [], 
              equippedItems: [], history: [], notes: [], // Agora é um array
  ...
  const handlePhotoUrlClick = () => {
    ...
    setModal({
      ...
      onConfirm: (newUrl) => {
        ...
        setModal({ isVisible: false, message: '', type: '', onConfirm: 
    ...
    <div className="min-h-screen bg-gray-900 text-gray-100 p-4 font-inter">
      ...
      <div className="max-w-4xl mx-auto bg-gray-800 rounded-lg shadow-2xl p-6 md:p-8 border border-gray-700">
        ...
        <section className="mb-8 p-4 bg-gray-700 rounded-xl shadow-inner border border-gray-600">
          ...
          >
            Status do Usuário
            <span>{character?.isUserStatusCollapsed ?
            '▼' : '▲'}</span>
          </h2>
        ...
        {user && selectedCharIdState && character && (
          <>
           ...
           {/* Informações do Personagem */}
            <section className="mb-8 p-6 bg-gray-700 rounded-xl shadow-inner border border-gray-600">
              ...
              >
               ...
               Informações do Personagem
                <span>{character.isCharacterInfoCollapsed ?
                '▼' : '▲'}</span>
              </h2>
              {!character.isCharacterInfoCollapsed && (
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 flex-grow w-full">
                    ...
                    <div>
                      ...
                      <input type="number" id="age" name="age" value={character.age === 0 ?
                      '' : character.age} onChange={handleChange} className="w-full p-2 bg-gray-600 border border-gray-500 rounded-md focus:ring-purple-500 focus:border-purple-500 text-white" disabled={user.uid !== character.ownerUid && !isMaster} />
                    </div>
                    <div>
                 ...
                 <div>
                      ...
                      <input type="number" id="level" name="level" value={character.level === 0 ?
                      '' : character.level} onChange={handleChange} className="w-full p-2 bg-gray-600 border border-gray-500 rounded-md focus:ring-purple-500 focus:border-purple-500 text-white" disabled={user.uid !== character.ownerUid && !isMaster} />
                    </div>
                    <div>
                      <label htmlFor="xp" className="block text-sm font-medium text-gray-300 mb-1">XP:</label>
              
                 <input type="number" id="xp" name="xp" value={character.xp === 0 ?
                 '' : character.xp} onChange={handleChange} className="w-full p-2 bg-gray-600 border border-gray-500 rounded-md focus:ring-purple-500 focus:border-purple-500 text-white" disabled={user.uid !== character.ownerUid && !isMaster} />
                    </div>
                  </div>
                </div>
              )}
           ...
           >
                Atributos Principais
                <span>{character.isMainAttributesCollapsed ?
                '▼' : '▲'}</span>
              </h2>
                    ...
                    <div className="flex items-center gap-2">
                     ...
                     <input
                        ...
                        data-attribute="hp"
                         ...
                         value={character.mainAttributes.hp.max === 0 ?
                         '' : character.mainAttributes.hp.max}
                        onChange={handleMainAttributeChange}
                        className="w-14 p-2 text-center bg-gray-700 border border-gray-500 rounded-md text-white text-xl font-bold"
                        disabled={!isMaster}
           ...
           />
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        type="number"
                         ...
                         name="current"
                        data-attribute="mp"
                        value={character.mainAttributes.mp.current === 0 ?
                        '' : character.mainAttributes.mp.current}
                        onChange={handleMainAttributeChange}
                        className="w-14 p-2 text-center bg-gray-700 border border-gray-500 rounded-md text-white text-xl font-bold"
                        disabled={user.uid !== character.ownerUid}
               ...
               />
                      ...
                      <input
                        ...
                        name="max"
                        data-attribute="mp"
                        value={character.mainAttributes.mp.max === 0 ?
                        '' : character.mainAttributes.mp.max}
                        onChange={handleMainAttributeChange}
                        className="w-14 p-2 text-center bg-gray-700 border border-gray-500 rounded-md text-white text-xl font-bold"
                        disabled={!isMaster}
           ...
           />
                 ...
                 <div key={attr} className="flex flex-col items-center p-2 bg-gray-600 rounded-md">
                      ...
                      <input
                        ...
                        id={attr}
                         ...
                         name={attr}
                        value={character.mainAttributes[attr] === 0 ?
                        '' : character.mainAttributes[attr]}
                 ...
                 />
                  ...
                  <p className="col-span-full text-sm text-gray-400 mt-2 text-center">
                    *A Iniciativa é baseada na Destreza ou Sabedoria (com custo 
                    de Mana para Sabedoria).
                  </p>
                </div>
              )}
            </section>
            ...
            <section className="mb-8 p-6 bg-gray-700 rounded-xl shadow-inner border border-gray-600">
              ...
              >
                Atributos Básicos
                <span>{character.isBasicAttributesCollapsed ?
                '▼' : '▲'}</span>
              </h2>
                            ...
                            <div className="flex items-center gap-2 text-xs flex-grow justify-end">
                              <div className="flex flex-col items-center">
                               ...
                               <span className="text-gray-400 text-xs text-center">Base</span>
                                <input type="number" value={attr.base === 0 ?
                                '' : attr.base} onChange={(e) => handleBasicAttributeChange('basicAttributes', key, 'base', e.target.value)} className="w-10 p-1 bg-gray-700 border border-gray-500 rounded-md text-white text-center" disabled={user.uid !== character.ownerUid && !isMaster} />
                              </div>
                              <div className="flex flex-col items-center">
               
                               <span className="text-gray-400 text-xs text-center">Perm.</span>
                                <input type="number" value={attr.permBonus === 0 ?
                                '' : attr.permBonus} onChange={(e) => handleBasicAttributeChange('basicAttributes', key, 'permBonus', e.target.value)} className="w-10 p-1 bg-gray-700 border border-gray-500 rounded-md text-white text-center" disabled={user.uid !== character.ownerUid && !isMaster} />
                              </div>
                              <div className="flex flex-col items-center">
               
                               <span className="text-gray-400 text-xs text-center">Cond.</span>
                                <input type="number" value={attr.condBonus === 0 ?
                                '' : attr.condBonus} onChange={(e) => handleBasicAttributeChange('basicAttributes', key, 'condBonus', e.target.value)} className="w-10 p-1 bg-gray-700 border border-gray-500 rounded-md text-white text-center" disabled={user.uid !== character.ownerUid && !isMaster} />
                              </div>
                              <div className="flex flex-col items-center">
               
                               <span className="text-gray-400 text-xs text-center">Total</span>
                                <input type="number" value={attr.total === 0 ?
                                '' : attr.total} readOnly className="w-10 p-1 bg-gray-700 border border-gray-500 rounded-md text-white font-bold cursor-not-allowed text-center" />
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-2 text-xs justify-between">
                            ...
                            <div className="flex items-center gap-2 text-xs flex-grow justify-end">
                              <div className="flex flex-col items-center">
                                 ...
                                 <span className="text-gray-400 text-xs text-center">Base</span>
                                <input type="number" value={attr.base === 0 ?
                                '' : attr.base} onChange={(e) => handleBasicAttributeChange('magicAttributes', key, 'base', e.target.value)} className="w-10 p-1 bg-gray-700 border border-gray-500 rounded-md text-white text-center" disabled={user.uid !== character.ownerUid && !isMaster} />
                              </div>
                              <div className="flex flex-col items-center">
               
                               <span className="text-gray-400 text-xs text-center">Perm.</span>
                                <input type="number" value={attr.permBonus === 0 ?
                                '' : attr.permBonus} onChange={(e) => handleBasicAttributeChange('magicAttributes', key, 'permBonus', e.target.value)} className="w-10 p-1 bg-gray-700 border border-gray-500 rounded-md text-white text-center" disabled={user.uid !== character.ownerUid && !isMaster} />
                              </div>
                              <div className="flex flex-col items-center">
               
                               <span className="text-gray-400 text-xs text-center">Cond.</span>
                                <input type="number" value={attr.condBonus === 0 ?
                                '' : attr.condBonus} onChange={(e) => handleBasicAttributeChange('magicAttributes', key, 'condBonus', e.target.value)} className="w-10 p-1 bg-gray-700 border border-gray-500 rounded-md text-white text-center" disabled={user.uid !== character.ownerUid && !isMaster} />
                              </div>
                              <div className="flex flex-col items-center">
            ...
            <section className="mb-8 p-6 bg-gray-700 rounded-xl shadow-inner border border-gray-600 relative"> {/* Added relative positioning */}
              ...
              >
                Inventário
                <span>{character.isInventoryCollapsed ?
                '▼' : '▲'}</span>
              </h2>
            ...
            <section className="mb-8 p-6 bg-gray-700 rounded-xl shadow-inner border border-gray-600">
              ...
              >
                ...
                <span>{character.isWalletCollapsed ?
                '▼' : '▲'}</span>
              </h2>
           ...
           <section className="mb-8 p-6 bg-gray-700 rounded-xl shadow-inner border border-gray-600 relative"> {/* Added relative positioning */}
              ...
              >
                 ...
                 Vantagens e Desvantagens
                <span>{character.isPerksCollapsed ?
                '▼' : '▲'}</span>
              </h2>
                           ...
                           <div className="flex justify-between items-center mb-1">
                              {perk.isCollapsed ? (
                                ...
                                >
                                   ...
                                   {perk.name ||
                                   'Vantagem Sem Nome'} {/* Alterado aqui */}
                                </span>
                              ) : (
                             ...
                             {!perk.isCollapsed && (
                              <>
                                ...
                                <div className="flex gap-3 text-sm text-gray-400 mt-2">
                                  ...
                                  <label className="flex items-center gap-1">
                                    <input type="checkbox" checked={perk.origin.manual} onChange={() => 
                                    handlePerkOriginChange('advantages', perk.id, 'manual')} className="form-checkbox text-purple-500 rounded" disabled={user.uid !== character.ownerUid && !isMaster} /> Manual
                                  </label>
                                </div>
                              ...
                              {perk.isCollapsed ? (
                               ...
                               <span 
                                ...
                                <input
                                  ...
                                  className="font-semibold text-lg w-full p-1 bg-gray-700 border border-gray-500 
                             ...
                             {!perk.isCollapsed && (
                              <>
                                ...
                                <div className="flex gap-3 text-sm text-gray-400 mt-2">
                                  ...
                                  <label className="flex items-center gap-1">
                                    <input type="checkbox" checked={perk.origin.manual} onChange={() => 
                                    handlePerkOriginChange('disadvantages', perk.id, 'manual')} className="form-checkbox text-purple-500 rounded" disabled={user.uid !== character.ownerUid && !isMaster} /> Manual
                                  </label>
                                </div>
            ...
            <section className="mb-8 p-6 bg-gray-700 rounded-xl shadow-inner border border-gray-600 relative"> {/* Added relative 
            positioning */}
              <h2 
                className="text-2xl font-bold text-yellow-300 mb-4 border-b-2 border-yellow-500 pb-2 cursor-pointer flex justify-between items-center"
              ...
              >
                Habilidades (Classe, Raça, Customizadas)
                 ...
                 <span>{character.isAbilitiesCollapsed ?
                '▼' : '▲'}</span>
              </h2>
            ...
            <section className="mb-8 p-6 bg-gray-700 rounded-xl shadow-inner border border-gray-600 relative"> {/* Added relative positioning */}
              ...
              >
                Especializações (Perícias)
                <span>{character.isSpecializationsCollapsed ?
                '▼' : '▲'}</span>
              </h2>
                           ...
                           <div className="flex justify-between items-center mb-1">
                            {spec.isCollapsed ? (
                                ...
                                >
                                   ...
                                   {spec.name ||
                                   'Especialização Sem Nome'} (Mod: {spec.modifier}, Bônus: {spec.bonus})
                                </span>
                            ) : (
            ...
            <section className="mb-8 p-6 bg-gray-700 rounded-xl shadow-inner border border-gray-600 relative"> {/* Added relative positioning */}
              ...
              >
                Itens Equipados
                <span>{character.isEquippedItemsCollapsed ?
                '▼' : '▲'}</span>
              </h2>
            ...
            <section className="mb-8 p-6 bg-gray-700 rounded-xl shadow-inner border border-gray-600">
              ...
              >
                História do Personagem
                <span>{character.isHistoryCollapsed ?
                '▼' : '▲'}</span>
              </h2>
                              ...
                              (
                                ...
                                <div className="flex flex-col items-center">
                                 ...
                                 <img
                                    ...
                                    style={{
                                      width: block.fitWidth ? 
                                      '100%' : (block.width ? `${block.width}px` : 'auto'),
                                      height: block.fitWidth ? 'auto' : (block.height ? `${block.height}px` : 'auto'),
                                      objectFit: 'contain'
        
                                     }}
                                    onError={(e) => { e.target.onerror = null;
                                    e.target.src = 'https://placehold.co/300x200/000000/FFFFFF?text=Erro+ao+carregar+imagem'; }}
                                  />
                                  {(user.uid === character.ownerUid || isMaster) && (
                                         ...
                                         <label className="flex items-center gap-1">
                                            Altura (px):
                                             ...
                                             <input
                                              ...
                                              className="w-20 p-1 
                                              bg-gray-700 border border-gray-500 rounded-md text-white text-center"
                                            />
                                          </label>
           ...
           >
                Anotações
                <span>{character.isNotesCollapsed ?
                '▼' : '▲'}</span>
              </h2>
           ...
           {/* Botões de Ação */}
            <div className="flex flex-wrap justify-center gap-4 mt-8">
              ...
              <button
                onClick={handleReset}
                className="px-8 py-3 bg-red-700 hover:bg-red-800 text-white font-bold rounded-lg shadow-lg 
                transition duration-200 ease-in-out transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-75"
                disabled={isLoading ||
                !user || (user.uid !== character.ownerUid && !isMaster)}
              >
                Resetar Ficha
              </button>
            </div>
